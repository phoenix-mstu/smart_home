...
function FloorHeatController(heatSource, heatValve, t_in, required_t, heaters) {
    var out_sensors = [];
    var subscriptions = ["wb-w1/" + t_in];
    for (var i in heaters) {
        if (!heaters.hasOwnProperty(i)) continue;
        out_sensors.push(heaters[i][1]);
        subscriptions.push("wb-w1/" + heaters[i][1]);
        heaters[i][0].on();
    }

    var switch_boiler_timer;
    var t_prev = 0;
    var switchValveIfRequired = function(newValue, devName, cellName) {
        ...
        if (t_round < required_t) {
            heatValve.on();
            if (!t_prev || t_prev !== t_round) {
                log('floor on. ' + t_round)
            }
            if (!heatSource.isOn() && !switch_boiler_timer) {
                switch_boiler_timer = setTimeout(function () {
                    heatSource.on();
                    switch_boiler_timer = false;
                    log('floor pump on.');
                }, 300000); // 5 min
            }
        } else {
            ...
            if (switch_boiler_timer) {
                clearTimeout(switch_boiler_timer);
                switch_boiler_timer = false;
            }
            if (heatSource.isOn()) {
                log('floor pump off.');
...
FloorDevicesHeatSource_Pump  = new Heater("_FLOOR_HEAT_SOURCE", 1, [BoilerPump]);
FloorDevicesHeatSource_Valve = new Heater("VALVE_FLOOR_LIVINGWALL", 1, []); // TODO: check off state with no consumers
...
// setTimeout(function () {
//     FloorHeatController(FloorDevicesHeatSource_Pump, FloorDevicesHeatSource_Valve, "28-8000001f1806", 35, [
